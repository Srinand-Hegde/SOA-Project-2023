...
# Training subset of the above training data for ice implementation
#Sampling
sample.size = round(0.1*nrow(adasyn_w_triggers_train))
adasyn_w_triggers_train_sub <- adasyn_w_triggers_train %>% sample_n(size = sample.size)
adasyn_w_triggers_test_sub <- adasyn_w_triggers_test %>% sample_n(size = sample.size)

# train the random forest model
classifier_rf_wt_sub <- randomForest(fraud ~ ., data = adasyn_w_triggers_train_sub, ntree = 100)

# predict probabilities on the test set
y_prob <- predict(classifier_rf_wt_sub, newdata = adasyn_w_triggers_test_sub, type = "prob")[,2]

# calculate ROC AUC and PR AUC
roc_obj <- roc(adasyn_w_triggers_test_sub$fraud, y_prob)
pr_obj <- pr.curve(scores.class0 = y_prob, weights.class0 = ifelse(adasyn_w_triggers_test_sub$fraud == "1", 1, 0))
...
# Confusion Matrix
c1 <- confusionMatrix(adasyn_w_triggers_test_sub$fraud, as.factor(y_pred), mode = "everything", positive = "1")
c <- rbind(c, c("RF adasyn_w_triggers_train", c1$overall[1], c1$byClass[c(1,2,5,6,7)], roc_obj$auc, pr_obj$auc.integral))
...
# Plotting model
plot(classifier_rf_wt_sub)
  
# Importance plot
importance(classifier_rf_wt_sub)
  
# Variable importance plot
varImpPlot(classifier_rf_wt_sub,top= 10)
```
...
shapley.rf <- list()
for (i in 1:nrow(adasyn_w_triggers_train_shap)) {
  shapley.rf[[i]] <- Shapley$new(predictor = predictor.rf, x.interest = adasyn_w_triggers_train_shap[i, -6], sample.size = 50)
}
```
...
# Training subset of the above training data for ice implementation
#Sampling
sample.size = round(0.1*nrow(smote_w_triggers_train))
smote_w_triggers_train_sub <- smote_w_triggers_train %>% sample_n(size = sample.size)
smote_w_triggers_test_sub <- smote_w_triggers_test %>% sample_n(size = sample.size)

# train the random forest model
classifier_rf_wt_sm_sub <- randomForest(fraud ~ ., data = smote_w_triggers_train_sub, ntree = 100)

# predict probabilities on the test set
y_prob <- predict(classifier_rf_wt_sm_sub, newdata = smote_w_triggers_test_sub, type = "prob")[,2]

# calculate ROC AUC and PR AUC
roc_obj <- roc(smote_w_triggers_test_sub$fraud, y_prob)
pr_obj <- pr.curve(scores.class0 = y_prob, weights.class0 = ifelse(smote_w_triggers_test_sub$fraud == "1", 1, 0))
...
# Confusion Matrix
c1 <- confusionMatrix(smote_w_triggers_test_sub$fraud, as.factor(y_pred), mode = "everything", positive = "1")
c <- rbind(c, c("RF smote_w_triggers_train", c1$overall[1], c1$byClass[c(1,2,5,6,7)], roc_obj$auc, pr_obj$auc.integral))

rm(c1)
# Plotting model
plot(classifier_rf_wt_sm_sub)
  
# Importance plot
importance(classifier_rf_wt_sm_sub)
  
# Variable importance plot
varImpPlot(classifier_rf_wt_sm_sub,top= 10)
