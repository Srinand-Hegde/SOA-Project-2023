...
### VIF for adasyn
```{r}
dt <- subset(adasyn_w_triggers_train,select=-fraud)
column_names <- names(dt)
vif_vector <- c()
for(name in column_names)
{
  index <- which(column_names == name)
  X <- dt[,-index]
  Y <- dt[,index]
  model <- lm(as.formula(paste(name, "~ .")), data = dt)
  alpha <- summary(model)$r.squared
  vif_value <- 1 / (1- alpha)
  vif_vector <- c(vif_vector,vif_value)
}

df <- data.frame(Var_names = column_names,Vif_vals = vif_vector)
arrange(df,desc(Vif_vals))
```

### VIF for smote
```{r}
dt <- subset(smote_w_triggers_train,select=-fraud)
column_names <- names(dt)
vif_vector <- c()
for(name in column_names)
{
...
### VIF for mwmote
```{r}
dt <- subset(mwmote_w_triggers_train,select=-fraud)
column_names <- names(dt)
vif_vector <- c()
for(name in column_names)
{
...
### VIF for rose
```{r}
dt <- subset(rose_w_triggers_train,select=-fraud)
column_names <- names(dt)
vif_vector <- c()
for(name in column_names)
{
...
c1
base_line_dt_F1_adasyn <- c1$byClass["F1"]
...
```
### function permutation feature imp for decision tree
```{r}
permutation_feature_importance_decision_Tree <- function(model,data,col_num,base_line_F1)
{
    ...
    for(i in 1:5)
    {
        col_values <- static_data[,col_num]
        shuffled_values <- sample(col_values)
        variable_data[,col_num] <- shuffled_values
       # print(variable_data)
        tree.pred <- predict(model,variable_data %>% dplyr::select(-c("fraud") ),type = "prob")[,2]
        roc_obj <- roc(variable_data$fraud, tree.pred)
       threshold <- coords(roc_obj, "best")[[1]]
tree.pred <- ifelse(tree.pred > threshold, 1, 0)
        c2 = confusionMatrix(as.factor(tree.pred),variable_data$fraud,mode = "everything",positive = "1")
        #print(c2)
        new_F1 <- c2$byClass["F1"]
        rm(c2)
        #print(new_accuracy)
        key <- key + abs(new_F1-base_line_F1)
    }
    return (key/5)
}
```
...
```{r}
col_names <- colnames(adasyn_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  ...
  col_num <- which(colnames(adasyn_w_triggers_test)== col)
  pfi <- permutation_feature_importance_decision_Tree(tree,adasyn_w_triggers_test,col_num,base_line_dt_F1_adasyn)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
arrange(values_df,desc(PFI))

top_10 <- values_df %>% arrange(desc(PFI)) %>% head(10)

ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Decision Forest Adasyn)", x = "PFI", y = "FEATURES")  +
...
base_line_dt_F1_smote <- c1$byClass["F1"]
...
```
### permutaion feature imp for smote
```{r}
col_names <- colnames(smote_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  if(col == "fraud"){
  ...
  col_num <- which(colnames(smote_w_triggers_test)== col)
  pfi <- permutation_feature_importance_decision_Tree(tree,smote_w_triggers_test,col_num,base_line_dt_F1_smote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Decision Forest Smote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```
...
base_line_dt_F1_mwmote <- c1$byClass["F1"]
...
```
### PERMUTATION FEATURE IMP FOR MWMOTE
```{r}
col_names <- colnames(mwmote_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  if(col == "fraud"){
  ...
  col_num <- which(colnames(mwmote_w_triggers_test)== col)
  pfi <- permutation_feature_importance_decision_Tree(tree,mwmote_w_triggers_test,col_num,base_line_dt_F1_mwmote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Decision Forest Mwmote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```
...
base_line_dt_F1_rose<- c1$byClass["F1"]
...
### PERMUTATION FEATURE IMP FOR ROSE
```{r}
col_names <- colnames(rose_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  if(col == "fraud"){
  ...
  col_num <- which(colnames(rose_w_triggers_test)== col)
  pfi <- permutation_feature_importance_decision_Tree(tree,rose_w_triggers_test,col_num,base_line_dt_F1_rose)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Decision Forest Rose)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
...
base_line_F1_adasyn <- c1$byClass["F1"]
...
### LIME for Random Forest ADASYN
```{r}
fraud.column <- which(colnames(adasyn_w_triggers_train) == "fraud")
X <- adasyn_w_triggers_train[, -fraud.column]
x_interest <- X[1, ]
predictor <- Predictor$new(model = classifier_rf_wt, data = X, y = adasyn_w_triggers_train[, fraud.column])
lime.explain <- LocalModel$new(predictor = predictor, x.interest = x_interest)
plot(lime.explain)
rm(X)
rm(fraud.column)
rm(x_interest)
```
### permutation feature importance random forest adasyn
```{r}
permutation_feature_importance <- function(model,data,col_num,base_line_F1)
{
  #print(base_line_accuracy)
    ...
    for(i in 1:5)
    {
        ...
        variable_data[,col_num] <- shuffled_values
       # print(variable_data)
        y_prob <- predict(model, newdata = variable_data, type = "prob")[,2]
        roc_obj <- roc(variable_data$fraud, y_prob)
        pr_obj <- pr.curve(scores.class0 = y_prob, weights.class0 = ifelse(variable_data$fraud == "1", 1, 0))
        y_pred <- ifelse(y_prob > threshold, 1, 0)
        c2 <- confusionMatrix(variable_data$fraud, as.factor(y_pred), mode = "everything", positive = "1")
        #print(c2)
        new_F1 <- c2$byClass["F1"]
        rm(c2)
        #print(new_accuracy)
...
for (col in col_names){
  ...
  col_num <- which(colnames(adasyn_w_triggers_test)== col)
  pfi <- permutation_feature_importance(classifier_rf_wt,adasyn_w_triggers_test,col_num,base_line_F1_adasyn)
  pfi2 <- permutation_feature_importance(classifier_rf_wt,adasyn_w_triggers_train,col_num,base_line_F1_adasyn)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
  v3 <-c(v3,pfi2)
}
values_df <- data.frame(COL = v1, PFI = v2,PFI2 = v3)
arrange(values_df,desc(PFI))
top_10 <- values_df %>% arrange(desc(PFI)) %>% head(10)

ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Random Forest Adasyn)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```
...
```{r}
plot_data <- top_10 %>%
  pivot_longer(cols = c(PFI, PFI2), names_to = "Value_Type", values_to = "Value")

ggplot(plot_data, aes(x = Value, y = reorder(COL, -Value), fill = Value_Type)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  labs(title = "Top 10 features by PFI and PFI2 (Random Forest Adasyn)",
       x = "Values", y = "Features") +
  scale_fill_manual(values = c("PFI" = "skyblue" , "PFI2" = "lightcoral"),
                    labels = c("PFI" = "test", "PFI2" = "train")) +  # Change legend labels
  theme_minimal() +
...
correlation_matrix <- cor( adasyn_w_triggers_test %>% select(-fraud))
###corrplot(correlation_matrix,method="color")

positive_threshold <- 0.5
negative_threshold <- -0.5

# Find pairs of variables with correlations above positive_threshold and below negative_threshold
high_correlation_pairs <- which(
  (correlation_matrix > positive_threshold & correlation_matrix != 1) |
  (correlation_matrix < negative_threshold),
  arr.ind = TRUE
)

# Convert row and column indices to variable names and include correlation values
variable_names <- colnames(correlation_matrix)
high_correlation_pairs <- data.frame(
  Variable1 = variable_names[high_correlation_pairs[, "row"]],
  Variable2 = variable_names[high_correlation_pairs[, "col"]],
  Correlation = correlation_matrix[high_correlation_pairs]
)

# Display the pairs of variables with correlations above the positive_threshold and below the negative_threshold
...
### dummy
```{r}
adasyn_w_triggers_train <- adasyn_w_triggers_train %>%
  mutate(DUMMY = claim_reported_delay_flag * .9)
adasyn_w_triggers_test <- adasyn_w_triggers_test %>%
  mutate(DUMMY = claim_reported_delay_flag * .9)
...
adasyn_w_triggers_train <- adasyn_w_triggers_train %>% select (-DUMMY)
```
...
### random forest (ADASYN) for SHAP
```{r}
selected_columns <- c("no_of_days_stayed","claim_reported_delay_flag","claim_duration_days","distance","approved_allowed_amount","fraud")
adasyn_w_triggers_train_shap <- adasyn_w_triggers_train[selected_columns]
adasyn_w_triggers_test_shap <- adasyn_w_triggers_test[selected_columns]
classifier_rf_wt_shap <- randomForest(fraud ~ ., data = adasyn_w_triggers_train_shap, ntree = 100)
```


### PDP for Random Forest(ADASYN)
```{r pdp using pdp library}
#top 5 features are "no_of_days_stayed", "Claim_reported_delay_flag", "claim_duration_days", "approved_allowed_amount", "distance")
pdp.rf.1 <- partial(classifier_rf_wt, pred.var = "no_of_days_stayed", plot = TRUE)
#partial(classifier_rf_wt, pred.var = "claim_reported_delay_flag", plot = TRUE)
#partial(classifier_rf_wt, pred.var = "claim_duration_days", plot = TRUE)
#partial(classifier_rf_wt, pred.var = "approved_allowed_amount", plot = TRUE)
#partial(classifier_rf_wt, pred.var = "distance", plot = TRUE)
plot(pdp.rf.1)
```
### ICE for Random Forest ADASYN
```{r ice rf adasyn}
ice.rf.1 <- partial(classifier_rf_wt_sub, pred.var = "no_of_days_stayed", plot = TRUE, ice = TRUE)
plot(ice.rf.1)
```

```{r}
ice_plot <- partial(object = classifier_rf_wt,
                    pred.var = "no_of_days_stayed",
                    grid.resolution = 100,
                    type = "auto",
                    ice = TRUE,
                    plot = TRUE)
...
```{r}
# Assuming you have a trained model (classifier_rf_w) and the dataset adasyn_w_triggers_test

# Feature name
feature_name <- "no_of_days_stayed"

# Create a sequence of feature values for the ICE plot
feature_values <- seq(min(adasyn_w_triggers_test[[feature_name]]), max(adasyn_w_triggers_test[[feature_name]]), length.out = 100)

# Initialize an empty matrix to store ICE curve data
ice_matrix <- matrix(0, nrow = nrow(adasyn_w_triggers_test), ncol = length(feature_values))

# Calculate ICE curves for each instance and feature value
for (i in 1:nrow(adasyn_w_triggers_test)) {
  for (j in 1:length(feature_values)) {
    # Create a copy of the data with the specified feature value
    data_copy <- adasyn_w_triggers_test
    data_copy[[feature_name]] <- feature_values[j]
    
    # Make predictions for the modified data point using your trained model
    predictions <- predict(classifier_rf_wt, newdata = data_copy)
    
    # Store the prediction for this instance and feature value
    ice_matrix[i, j] <- predictions[i]
  }
}

# Create the ICE plot
plot(feature_values, t(ice_matrix), type = "l", xlab = feature_name, ylab = "Predicted Outcome", 
     main = paste("ICE Plot for", feature_name))

```

```{r}
n_train <- floor(0.4 * nrow(adasyn_w_triggers_train))
train_indices <- sample(1:nrow(adasyn_w_triggers_train), size = n_train, replace = FALSE)
training_data <- adasyn_w_triggers_train[train_indices, ]
classifier_rf <- randomForest(fraud ~ ., data = training_data, ntree = 100)

ice.rf.1 <- partial(classifier_rf, pred.var = "no_of_days_stayed", plot = TRUE, ice = TRUE)
plot(ice.rf.1)
```


### SHAP feature importance Random Forest
```{r shap feature importance}
# Preprocessing for shap implementation
selected_columns <- c("no_of_days_stayed","claim_reported_delay_flag","claim_duration_days","distance","approved_allowed_amount","fraud")
adasyn_w_triggers_train_shap <- adasyn_w_triggers_train[selected_columns]
adasyn_w_triggers_test_shap <- adasyn_w_triggers_test[selected_columns]
classifier_rf_wt_shap <- randomForest(fraud ~ ., data = adasyn_w_triggers_train_shap, ntree = 100)

predictor.rf <- Predictor$new(model = classifier_rf_wt_shap, data = adasyn_w_triggers_train_shap %>% select(-!!sym("fraud")), y = adasyn_w_triggers_train_shap %>% select(!!sym("fraud"))%>%pull())

shapley.rf <- list()
n <- nrow(adasyn_w_triggers_train_shap)/2
for (i in 1:n) {
  shapley.rf[[i]] <- Shapley$new(predictor = predictor.rf, x.interest = adasyn_w_triggers_train_shap[i, -6], sample.size = 10)
}
plot(shapley.rf[[1]])


shap_result = shap.values(xgb_model = classifier_rf_wt_shap,
                          X_train = adasyn_w_triggers_train_shap)

shap.nodays <- c()
shap.claim.delay.flag <- c()
for(j in 1:length(shapley.rf)){
  shap.nodays[j] <- abs(shapley.rf[[j]]$results[6, 4])
  shap.claim.delay.flag[j] <- abs(shapley.rf[[j]]$results[7, 4])
}
shap.nodays.imp <- sum(shap.nodays)/length(shap.nodays)
shap.claim.delay.flag.imp <- sum(shap.claim.delay.flag)/length(shap.claim.delay.flag)
```
...
```{r}
# train the random forest model
classifier_rf_wt_sm <- randomForest(fraud ~ ., data = smote_w_triggers_train, ntree = 100)

# predict probabilities on the test set
y_prob <- predict(classifier_rf_wt_sm, newdata = smote_w_triggers_test, type = "prob")[,2]
...
base_line_rf_F1_smote <- c1$byClass["F1"]
...
### permutation feature importance random forest smote
```{r}
col_names <- colnames(smote_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  ...
  col_num <- which(colnames(smote_w_triggers_test)== col)
  pfi <- permutation_feature_importance(classifier_rf_wt_sm,smote_w_triggers_test,col_num,base_line_rf_F1_smote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Random Forest Smote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### Random forest (SMOTE) for shap
```{r}
selected_columns <- c("no_of_days_stayed","claim_reported_delay_flag","claim_duration_days","birth_date","approved_allowed_amount","fraud")
smote_w_triggers_train_shap <- smote_w_triggers_train[selected_columns]
smote_w_triggers_test_shap <- smote_w_triggers_test[selected_columns]
classifier_rf_wt_sm_shap <- randomForest(fraud ~ ., data = smote_w_triggers_train_shap, ntree = 100)
```

### PDP for Random Forest(SMOTE)

```{r pdp random forest smote}
pdp.rf.2 <- partial(classifier_rf_wt_sm, pred.var = "approved_allowed_amount", plot = TRUE)
plot(pdp.rf.2)
```

### ICE for Random Forest SMOTE
```{r ice rf smote}
#Creating the predictor object used to store the model
predictor = Predictor$new(classifier_rf_wt_sm_sub, data = adasyn_w_triggers_test %>% select(-!!sym("fraud")), y = adasyn_w_triggers_test %>% select(!!sym("fraud")) %>% pull())

# Calculating the effect of a feature on the response
rf.ice.2 <- FeatureEffect$new(predictor, feature = "approved_allowed_amount", method = "pdp+ice")
rf.ice.2$plot()

#ggplot(featureEffect.data, aes(claim_reported_delay_flag)) +
#  geom_line(aes(y = .value), color = "blue") +
#  labs(title = "PDP for claim_reported_delay_flag", x = "claim_reported_delay_flag", y = "Degree of partial dependence"
```
...
```{r}
# train the random forest model
classifier_rf_wt_mw <- randomForest(fraud ~ ., data = mwmote_w_triggers_train, ntree = 100)

# predict probabilities on the test set
y_prob <- predict(classifier_rf_wt_mw, newdata = mwmote_w_triggers_test, type = "prob")[,2]
...
c1
base_line_rf_F1_mwmote <- c1$byClass["F1"]
...
### permutation feature importance random forest mwmote
```{r}
col_names <- colnames(mwmote_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  ...
  col_num <- which(colnames(mwmote_w_triggers_test)== col)
  pfi <- permutation_feature_importance(classifier_rf_wt_mw,mwmote_w_triggers_test,col_num,base_line_rf_F1_mwmote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Random Forest Mwmote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### Random Forest(mwsmote) for shap
```{r}
selected_columns <- c("no_of_days_stayed","claim_reported_delay_flag","claim_duration_days","distance","approved_allowed_amount","fraud")
mwmote_w_triggers_train_shap <- mwmote_w_triggers_train[selected_columns]
mwmote_w_triggers_test_shap <- mwmote_w_triggers_test[selected_columns]
classifier_rf_wt_mw_shap <- randomForest(fraud ~ ., data = mwmote_w_triggers_train_shap, ntree = 100)
```

### PDP for random forest(MWMOTE)
```{r pdp random forest mwmote}
pdp.rf.3 <- partial(classifier_rf_wt_mw, pred.var = "approved_allowed_amount", plot = TRUE)
plot(pdp.rf.3)
```

```{r}
ice_results <- partial(classifier_rf_wt_mw, pred.var = "no_of_days_stayed", plot = TRUE, ice = TRUE)
...
```{r}
# train the random forest model
classifier_rf_wt_rs <- randomForest(fraud ~ ., data = rose_w_triggers_train, ntree = 100)

# predict probabilities on the test set
y_prob <- predict(classifier_rf_wt_rs, newdata = rose_w_triggers_test, type = "prob")[,2]
...
base_line_rf_F1_rose <- c1$byClass["F1"]
...
### permutation feature importance random forest rose
...
for (col in col_names){
  ...
  col_num <- which(colnames(rose_w_triggers_test)== col)
  pfi <- permutation_feature_importance(classifier_rf_wt_rs,rose_w_triggers_test,col_num,base_line_rf_F1_rose)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Random Forest Rose)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```


### Random Forest(rose) for shap
```{r}
selected_columns <- c("no_of_days_stayed","claim_reported_delay_flag","claim_duration_days","birth_date","approved_allowed_amount","fraud")
rose_w_triggers_train_shap <- rose_w_triggers_train[selected_columns]
rose_w_triggers_test_shap <- rose_w_triggers_test[selected_columns]
classifier_rf_wt_rs_shap <- randomForest(fraud ~ ., data = rose_w_triggers_train_shap, ntree = 100)
```

### PDP for random forest(ROSE)
```{r}
pdp.rf.4 <- partial(classifier_rf_wt_rs, pred.var = "approved_allowed_amount", plot = TRUE)
plot(pdp.rf.4)
```
...
# Train the model
xgb.wt.adsyn <- xgb.train(data = dtrain, nrounds = 15)

# Use the model to make predictions on test data
pred_y <- predict(xgb.wt.adsyn, dtest)
...
c1
base_line_XG_F1_adasyn <- c1$byClass["F1"]

c <- rbind(c, c("XGB adasyn_w_triggers_train", c1$overall[1], c1$byClass[c(1,2,5,6,7)], roc_auc, pr_auc))


xgb.plot.importance(xgb.importance(names(adasyn_w_triggers_train %>% dplyr::select(-c("fraud"))),model = xgb.wt.adsyn))

```
### LIME for Xgboost Adasyn
```{r}
fraud.column <- which(colnames(adasyn_w_triggers_train) == "fraud")
X <- adasyn_w_triggers_train[, -fraud.column]
x_interest <- X[1, ]

predictor = Predictor$new(xgb.wt.adsyn, 
                          data = X, 
                          y = adasyn_w_triggers_train[fraud.column],
...
# Create the feature matrix X by excluding the "fraud" column
X <- adasyn_w_triggers_train[, -fraud.column]

# Select a specific data point for explanation
x_interest <- X[1, ]

# Load the necessary functions for DALEX and LIME
library(DALEX)
library(lime)

# Create a predictor object for XGBoost
...
# Create a LIME explainer using the predictor and the data point of interest
lime.explain <- local_model(predictor = predictor, x = x_interest)

# Generate a plot to visualize the LIME explanation
plot(lime.explain)
...
### permutation feature importance function
```{r}
permutation_feature_importance <- function(model,data,col_num,base_line_F1)
{
  #print(base_line_accuracy)
    ...
    for(i in 1:5)
    {
        ...
        shuffled_values <- sample(col_values)
        variable_data[,col_num] <- shuffled_values
        dtest <- xgb.DMatrix(data = data.matrix(variable_data%>% dplyr::select(-c(fraud))), label= variable_data$fraud)
       # print(variable_data)
        pred_y <- predict(model,dtest)
        
        roc_obj <- roc(variable_data$fraud, pred_y)
        roc_auc <- auc(roc_obj)

        pr_obj <- pr.curve(variable_data$fraud, pred_y)
        pr_auc <- pr_obj$auc.integral

       threshold <- coords(roc_obj, "best")[[1]]
       tree.pred <- ifelse(pred_y > threshold, 1, 0)
        
       c2 <-confusionMatrix(variable_data$fraud, as.factor(tree.pred), mode = "everything", positive = "1")
        #print(c2)
...
for (col in col_names){
  ...
  col_num <- which(colnames(adasyn_w_triggers_test)== col)
  pfi <- permutation_feature_importance(xgb.wt.adsyn,adasyn_w_triggers_test,col_num,base_line_XG_F1_adasyn)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Xg Boost Adasyn)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))

```

### SHAP Feature Importance xgb adasyn
```{r}
shap_result = shap.values(xgb_model = xgb.wt.adsyn,
                          X_train = dtrain)
...
### PDP for XGBoost adasyn
```{r pdp xgb adasyn}
# Creating the predictor object used to store the model
predictor = Predictor$new(xgb.wt.adsyn, data = adasyn_w_triggers_test %>% select(-!!sym("fraud")), y = adasyn_w_triggers_test %>% select(!!sym("fraud")) %>% pull(),
                            predict.fun=function(model, newdata){
...
# Calculating the effect of a feature on the response
featureEffect = FeatureEffect$new(predictor, feature = "claim_reported_delay_flag", method = "pdp")
...
pdp.data.claimcount <- pdp_plot_data(xgb.wt.adsyn, adasyn_w_triggers_test, "claim_count_flag", "fraud", "ice")
```
...
# Train the model
xgb.wt.sm <- xgb.train(data = dtrain, nrounds = 15)

# Use the model to make predictions on test data
pred_y <- predict(xgb.wt.sm, dtest)
...
# Confusion Matrix
 ...
 base_line_XG_F1_smote <- c1$byClass["F1"]
# c <- rbind(c, c("XGB smote_w_triggers_train", c1$overall[1], c1$byClass[c(1,2,5,6,7)], roc_auc, pr_auc))
# rm(c1)

xgb.plot.importance(xgb.importance(names(smote_w_triggers_train %>% dplyr::select(-c("fraud"))),model = xgb.wt.sm))

```
### permutation feature importance xg boost smote
```{r}
col_names <- colnames(smote_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  ...
  col_num <- which(colnames(smote_w_triggers_test)== col)
  pfi <- permutation_feature_importance(xgb.wt.sm,smote_w_triggers_test,col_num,base_line_XG_F1_smote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Xg Boost Smote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### SHAP Feature Importance xgb smote
```{r}
shap_result = shap.values(xgb_model = xgb.wt.sm,
                          X_train = dtrain)
...
### PDP for XGBoost smote
```{r pdp xgb smote}
# Creating the predictor object used to store the model
predictor = Predictor$new(xgb.wt.sm, data = adasyn_w_triggers_test %>% select(-!!sym("fraud")), y = adasyn_w_triggers_test %>% select(!!sym("fraud")) %>% pull(),
                            predict.fun=function(model, newdata){
                              results <- predict(model, as.matrix(newdata))
                              return(results)
                            }
...
# Calculating the effect of a feature on the response
featureEffect = FeatureEffect$new(predictor, feature = "claim_duration_days", method = "pdp")
featureEffect.data <- featureEffect$results

ggplot(featureEffect.data, aes(claim_duration_days)) +
  geom_line(aes(y = .value), color = "blue") +
  labs(title = "PDP for claim_duration_days", x = "claim_duration_days", y = "Degree of partial dependence")
...
# Train the model
xgb.wt.mw <- xgb.train(data = dtrain, nrounds = 15)

# Use the model to make predictions on test data
pred_y <- predict(xgb.wt.mw, dtest)
...
base_line_XG_F1_mwmote <- c1$byClass["F1"]
c <- rbind(c, c("XGB mwmote_w_triggers_train", c1$overall[1], c1$byClass[c(1,2,5,6,7)], roc_auc, pr_auc))
rm(c1)

xgb.plot.importance(xgb.importance(names(mwmote_w_triggers_train %>% dplyr::select(-c("fraud"))),model = xgb.wt.mw))
...
for (col in col_names){
  ...
  col_num <- which(colnames(mwmote_w_triggers_test)== col)
  pfi <- permutation_feature_importance(xgb.wt.mw,mwmote_w_triggers_test,col_num,base_line_XG_F1_mwmote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Xg Boost Mwmote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### PDP for XGBoost mwmote
```{r pdp xgb mwmote}
# Creating the predictor object used to store the model
predictor = Predictor$new(xgb.wt.mw, data = adasyn_w_triggers_test %>% select(-!!sym("fraud")), y = adasyn_w_triggers_test %>% select(!!sym("fraud")) %>% pull(),
                            predict.fun=function(model, newdata){
                              results <- predict(model, as.matrix(newdata))
                              return(results)
                            }
...
### SHAP Feature Importance xgb mwmote
```{r}
shap_result = shap.values(xgb_model = xgb.wt.mw,
                          X_train = dtrain)
...
# Train the model
xgb.rose <- xgb.train(data = dtrain, nrounds = 15)

# Use the model to make predictions on test data
pred_y <- predict(xgb.rose, dtest)
...
xgb.plot.importance(xgb.importance(names(rose_wo_triggers_train %>% dplyr::select(-c("fraud"))),model = xgb.rose))
...
# Train the model
xgb.wt.rose <- xgb.train(data = dtrain, nrounds = 15)

# Use the model to make predictions on test data
pred_y <- predict(xgb.wt.rose, dtest)
...
base_line_XG_F1_rose <- c1$byClass["F1"]
c <- rbind(c, c("XGB rose_w_triggers_train", c1$overall[1], c1$byClass[c(1,2,5,6,7)], roc_auc, pr_auc))
rm(c1)

xgb.plot.importance(xgb.importance(names(rose_w_triggers_train %>% dplyr::select(-c("fraud"))),model = xgb.wt.rose))
...
for (col in col_names){
  ...
  col_num <- which(colnames(rose_w_triggers_test)== col)
  pfi <- permutation_feature_importance(xgb.wt.rose,rose_w_triggers_test,col_num,base_line_XG_F1_rose)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (Xg Boost Rose)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```


### PDP for XGBoost rose
```{r pdp xgb rose}
# Creating the predictor object used to store the model
predictor = Predictor$new(xgb.wt.rose, data = adasyn_w_triggers_test %>% select(-!!sym("fraud")), y = adasyn_w_triggers_test %>% select(!!sym("fraud")) %>% pull(),
                            predict.fun=function(model, newdata){
                              results <- predict(model, as.matrix(newdata))
                              return(results)
                            }
...
### SHAP Feature Importance xgb ROSE
```{r}
shap_result = shap.values(xgb_model = xgb.wt.adsyn,
                          X_train = dtrain)
...
# Fit the GLM model
glm.wt.adsyn <- glm(fraud ~ ., data = adasyn_w_triggers_train, family = binomial(link = "logit"))

# Testing Set
test_prob <- predict(glm.wt.adsyn, newdata = adasyn_w_triggers_test %>% dplyr::select(-c("fraud")), type = "response")
...
base_line_glm_F1_adasyn <- c1$byClass["F1"]
...
permutation_feature_importance <- function(model,data,col_num,base_line_F1)
{
  #print(base_line_accuracy)
    ...
    for(i in 1:5)
    {
        ...
        variable_data[,col_num] <- shuffled_values
       # print(variable_data)
        test_prob <- predict(model, newdata = variable_data %>% dplyr::select(-c("fraud")), type = "response")

# Get the ROC curve and calculate the ROC AUC
roc_obj <- roc(variable_data$fraud, test_prob)
roc_auc <- auc(roc_obj)

# Get the PR curve and calculate the PR AUC
pr_obj <- pr.curve(variable_data$fraud, test_prob)
pr_auc <- pr_obj$auc.integral
...
model_glm_test_pred <- ifelse(test_prob > threshold, 1, 0)
        ...
        c2 <- confusionMatrix(variable_data$fraud, as.factor(model_glm_test_pred), mode = "everything", positive = "1")
        #print(c2)
        new_F1 <- c2$byClass["F1"]
        rm(c2)
        #print(new_accuracy)
...
for (col in col_names){
  ...
  col_num <- which(colnames(adasyn_w_triggers_test)== col)
  pfi <- permutation_feature_importance(glm.wt.adsyn,adasyn_w_triggers_test,col_num,base_line_glm_F1_adasyn)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (GLM Adasyn)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### PDP for GLM ADASYN  
```{r pdp glm adasyn}
pdp.glm.1 <- partial(glm.wt.adsyn, pred.var = "approved_allowed_amount", plot = TRUE)
plot(pdp.glm.1)
```
...
# Fit the GLM model
glm.wt.sm <- glm(fraud ~ ., data = smote_w_triggers_train, family = binomial(link = "logit"))

# Testing Set
test_prob <- predict(glm.wt.sm, newdata = smote_w_triggers_test %>% dplyr::select(-c("fraud")), type = "response")
...
for (col in col_names){
  ...
  col_num <- which(colnames(smote_w_triggers_test)== col)
  pfi <- permutation_feature_importance(glm.wt.sm,smote_w_triggers_test,col_num,base_line_glm_F1_smote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (GLM Smote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### PDP for GLM SMOTE  
```{r pdp glm smote}
pdp.glm.2 <- partial(glm.wt.sm, pred.var = "no_of_days_stayed", plot = TRUE)
plot(pdp.glm.2)
```
...
# Fit the GLM model
glm.wt.mw <- glm(fraud ~ ., data = mwmote_w_triggers_train, family = binomial(link = "logit"))

# Testing Set
test_prob <- predict(glm.wt.mw, newdata = mwmote_w_triggers_test %>% dplyr::select(-c("fraud")), type = "response")
...
base_line_glm_F1_mwmote <- c1$byClass["F1"]
...
for (col in col_names){
  ...
  col_num <- which(colnames(mwmote_w_triggers_test)== col)
  pfi <- permutation_feature_importance(glm.wt.mw,mwmote_w_triggers_test,col_num,base_line_glm_F1_mwmote)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (GLM Mwmote)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### PDP for GLM MWMOTE  
```{r pdp glm mwmote}
pdp.glm.3 <- partial(glm.wt.mw, pred.var = "claim_duration_days", plot = TRUE)
plot(pdp.glm.3)
```
...
# Fit the GLM model
glm.wt.rose <- glm(fraud ~ ., data = rose_w_triggers_train, family = binomial(link = "logit"))

# Testing Set
test_prob <- predict(glm.wt.rose, newdata = rose_w_triggers_test %>% dplyr::select(-c("fraud")), type = "response")
...
base_line_glm_F1_rose <- c1$byClass["F1"]
...
```
### permutation feature importance for glm rose
```{r}
col_names <- colnames(rose_w_triggers_test)
v1 <- c()
v2 <- c()
for (col in col_names){
  ...
  col_num <- which(colnames(rose_w_triggers_test)== col)
  pfi <- permutation_feature_importance(glm.wt.rose,rose_w_triggers_test,col_num,base_line_glm_F1_rose)
  v1 <-c(v1,col)
  v2 <-c(v2,pfi)
}
values_df <- data.frame(COL = v1, PFI = v2)
...
ggplot(top_10, aes(x = PFI, y = reorder(COL, PFI))) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 features by PFI (GLM Rose)", x = "PFI", y = "FEATURES")  +
  theme(axis.text.y = element_text(hjust = 1))
```

### PDP for GLM ROSE  
```{r pdp glm rose}
pdp.glm.4 <- partial(glm.wt.rose, pred.var = "claim_count_flag", plot = FALSE)
plot(pdp.glm.4)
```
